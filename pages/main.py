...
with st.container():
    ...
    with col1:
        st.markdown("**모델**", help="AI 모델 선택")
        model = st.selectbox(
            "모델 선택",
            ["gpt-4", "gpt-3.5-turbo", "gemini-2.5-flash", "claude-3"],
            index=2,
            label_visibility="collapsed"
        )
    
    with col2:
        st.markdown("**도구**", help="AI 도구 사용")
        use_tools = st.checkbox("도구 사용", value=True, label_visibility="collapsed")
    
    with col3:
        st.markdown("**Temp**", help="창의성 조절")
